...
def create_office():
    ...
    try:
        ...
        return make_response(jsonify({"status": 400, "error":
                                      "Must provide id, name and type"}), 400)

    newoffice = OfficeModel(name=name, type=type, id=id)
    newoffice.save_office()

    return make_response(jsonify({"status": 201, "data": [{"type": type,
                                  "name": name, "id": id}]}), 201)


@endpoint.route('/offices', methods=['GET'])
def get_all_offices():
    OFFICES = OfficeModel.view_all_offices()
...
def edit_office(id):
    ...
    if office:
        return make_response(jsonify({"status": 200, "data": [{"id": id,
                                      "name": name}]}), 200)
